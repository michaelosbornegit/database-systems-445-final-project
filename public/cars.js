class Cars {
  constructor() {
    ...
    const userAction = async () => {
      const response = await fetch(proxyUrl + 'https://osbornem-database-project.herokuapp.com/getcars');
      ...
      json.forEach((joinedRow) => {
        ...
        row.addEventListener('click', () => {
          sessionStorage.setItem('selectedCar', joinedRow.RACE.RaceID);
          window.location.assign(location.pathname.substring(0, location.pathname.length - 'cars.html'.length) + 'single-race-view.html');
        });
